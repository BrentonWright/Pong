...
credit_size = 15
delay_counter = 0 
delay_size = 1 # in seconds
volley_counter = 0
mult = 0
allowance = 8 # added to paddle length to HIT BALL when it looks like it SHOULD HIT IT
direction = "Random"
...
def reset_game():
    global direction, paddle1_pos, paddle2_pos, paddle1_vel, paddle2_vel, is_game_ended 
    #resetting direction
    direction = "Random"
    unlock_paddles()
...
def end_game():
    ...
    is_game_ended = True # and LOCKS paddles if True
    is_playing = False
    vel = [0,0] #pauses ball
...
def draw(canvas):
    global direction, allowance, volley_counter, score1, score2, paddle1_pos, paddle2_pos, ball_pos, ball_vel, paddle1_vel, paddle1_move, paddle2_vel, paddle2_move, vel, is_game_ended, is_playing
    ...
    if (ball_pos[0] <= 0 + PAD_WIDTH + BALL_RADIUS):
        ...
        if (ball_pos[1] > (paddle1_pos[0][1] - allowance)) and (ball_pos[1] < (paddle1_pos[3][1] + allowance) and (is_game_ended == False)):
            ...
            if(score2 < score_limit ): # Compare Score with ScoreLimit: if score2 is under score_limit, then
                ...
                if(score2 < score_limit): #if score2 is still LESS than score_limit
                    ...
                    score_delay_timer.start()
                    direction = "Right" #Set direction towards Winner
            else:
                end_game()
    ...
    if (ball_pos[0] >= WIDTH - PAD_WIDTH - (BALL_RADIUS)):
        # determine whether paddle and ball collide
        if (ball_pos[1] > (paddle2_pos[0][1] - allowance)) and (ball_pos[1] < (paddle2_pos[3][1] + allowance) and (is_game_ended == False)): 
            ...
            if(score1 < score_limit ): # Compare Score with ScoreLimit: if score2 is under score_limit, then
                ...
                if(score1 < score_limit): #if score2 is still LESS than score_limit
                    reset_ball_offscreen() #Hack to fix ball remaining in gutter
                    score_delay_timer.start()
                    direction = "Left" #Set direction towards Winner  
            else:
                end_game()
    ...
    canvas.draw_text(str(score2), ((WIDTH / 4) * 3 -20, HEIGHT / 4), 100, 'White', 'sans-serif') # Displaying/Offsetting text
